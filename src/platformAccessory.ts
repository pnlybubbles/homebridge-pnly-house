...
import { Humidifier, HumidifierState, RawActive } from "./machine/humidifier";
...
export class SwitchBotCustomHumidifier {
  ...
  constructor(
    ...
    this.accessory
      ...
      .setCharacteristic(
        ...
        this.accessory.context.device?.deviceName ?? "Unknown"
      )
      .setCharacteristic(
        this.platform.Characteristic.SerialNumber,
        this.accessory.context.device?.deviceId ??
          `Unknown-${this.accessory.UUID}`
      );
  ...
  handleRelativeHumidityHumidifierThresholdGet(
    callback: CharacteristicGetCallback
  ): void {
    this.platform.log.debug(
      "Triggered GET RelativeHumidityHumidifierThreshold"
    );

    callback(null, this.humidifier.getTargetHumidity());
  }

  handleRelativeHumidityHumidifierThresholdSet(
    value: CharacteristicValue,
    callback: CharacteristicSetCallback
  ): void {
    this.platform.log.debug(
      "Triggered SET RelativeHumidityHumidifierThreshold",
      value
    );

    callback(null);

    void this.humidifier.setTargetHumidity(value as number);
  }

  handleCurrentHumidifierDehumidifierStateGet(
    callback: CharacteristicGetCallback
  ): void {
  ...
  handleTargetHumidifierDehumidifierStateGet(
    callback: CharacteristicSetCallback
  ): void {
  ...
  handleTargetHumidifierDehumidifierStateSet(
    value: unknown,
    callback: CharacteristicSetCallback
  ): void {
    this.platform.log.debug(
      "Triggered SET TargetHumidifierDehumidifierState:",
      value
    );
  ...
  handleActiveSet(
    ...
    void this.humidifier.setActive(value as RawActive);
  }
}
