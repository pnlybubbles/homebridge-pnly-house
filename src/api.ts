...
export function getDevices({ token }: RequestConfig) {
  if (process.env.NODE_ENV === 'development') {
    return nextTick<Response<Devices>>(JSON.parse(
  ...
  return axios.get<Response<Devices>>(`${SWITCHBOT_API_ENDPOINT}/devices`, {
    headers: {
      Authorization: token,
    },
  }).then(v => v.data);
...
export function commandDevice({ token, deviceId, command, parameter, commandType }: RequestConfig & CommandProps) {
  return axios.post<Response<Record<string, never>>>(`${SWITCHBOT_API_ENDPOINT}/devices/${deviceId}/commands`, {
    command,
    parameter,
    commandType,
  }, {
